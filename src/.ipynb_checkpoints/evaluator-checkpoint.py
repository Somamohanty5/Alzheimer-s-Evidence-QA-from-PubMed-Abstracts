...
from .retriever import RetrievalPipeline, CorpusData


# ---------- PubMed enrichment ----------

def fetch_pubmed_records(pmids: Iterable[str], email: str, tool_name: str = "alz-evidence-qa") -> Dict[str, Dict[str, str]]:
    """
    Fetch titles and abstracts for PMIDs using NCBI E-utilities.
...
def select_best_dense_model(
    pipeline: RetrievalPipeline,
    dev_questions: List[Dict[str, Any]],
    dense_model_names: List[str],
) -> str:
    """Pick the dense model with the best nDCG@10 on the dev AD subset."""
    scores: Dict[str, float] = {}

    for model_name in dense_model_names:
        def retrieve_with_dense(query_text: str, topk: int = 20) -> List[str]:
            return pipeline.retrieve_with_dense_rerank(
                query_text,
                dense_name=model_name,
                topk=topk,
                pool_k=300,
            )

        metrics = evaluate_retrieval_variant(
            questions=dev_questions,
            retrieve_fn=retrieve_with_dense,
            variant_name=f"{model_name.upper()} fusion+rerank (dev)",
            topk=20,
        )
        scores[model_name] = metrics.ndcg_at_10

    best_model = max(scores.items(), key=lambda item: item[1])[0]
    print("Best dense model on dev:", best_model)
    return best_model
...
def select_top_sentences_for_pmids(
    ...
    """
    Select informative sentences from candidate documents using a simple TF-IDF
    relevance model over sentences.
    """
...
def evaluate_qa(
    ...
    answers_path = RUNS_DIR / f"{save_prefix}_{timestamp}.jsonl"
    metrics_path = RUNS_DIR / f"{save_prefix}_metrics_{timestamp}.json"
...
def preview_question_with_titles(
    ...
    - question
    - top PMIDs + titles
    - LLM answer + cited PMIDs

    Uses the given dense model name inside the retrieval pipeline.
    """
    ranked_pmids = pipeline.retrieve_with_dense_rerank(
        question["body"],
        dense_name=dense_name,
